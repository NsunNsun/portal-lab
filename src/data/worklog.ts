...
export const worklog: Worklog = {
  ...
  stages: [
    ...
    },
    {
      ...
      time: 'TODO',
      human:
        'Прогнал сцену вручную и нашёл пять шероховатостей: метки наблюдателя/спасателя стоят в узкой колонке имени и разносят строку; история портала растёт без ограничения; системная запись «Прошёл час» не привязана к номеру часа смены; тосты копятся стопкой и закрывают экран; закрытие портала с людьми внутри не оставляет следа в истории. Формулу риска и правила действий менять запретил.',
      ai:
        'Перенёс людей в колонку «Внутри» (число существ + приглушённый список «наблюдатель, спасатель»; PeopleBadges заменил на PeopleInside/peopleInside, удалил мёртвый Badge). История портала теперь показывает последние 8 записей с кнопкой «Показать все (N)» (компонент PortalHistory, keyed по id портала). Tick и тост часа получили номер часа смены («Прошёл час — час N смены»). Стопку тостов ограничил тремя последними (slice(-3)). Закрытие портала с кем-то внутри пишет вторую запись в history (closeContentsNote) — тот же принцип скрытых данных, что и у схлопывания: неразведанный портал не раскрывает содержимое; вынес общий insideList. Добавил 4 теста (45 всего) и обновил CLAUDE.md. Проверку в браузере оставил за человеком.',
      prompt:
        'Проект portal-lab. Пять правок по результатам ручного тестирования. Формулу риска и правила действий не трогаем. Колонка «Существа» → «Внутри» с людьми под числом; история — последние 8 с «Показать все»; номер часа смены в журнале и тостах; не больше трёх тостов; вторая запись в истории при закрытии с кем-то внутри без раскрытия скрытого.',
    },
  ],
  decisions: [
    {
